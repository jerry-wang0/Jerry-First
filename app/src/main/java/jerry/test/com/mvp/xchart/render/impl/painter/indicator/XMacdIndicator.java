...
import cn.nextop.erebor.mid.app.mvc.support.widget.xchart.render.impl.painter.indicator.config.XMacdConfig;
import cn.nextop.erebor.mid.app.mvc.support.widget.xchart.render.impl.painter.indicator.index.XMacdIndex;
...
import static cn.nextop.erebor.mid.app.mvc.support.widget.xchart.formula.XChartFormula.macd;
 ...
 */
public class XMacdIndicator extends AbstractXIndicator {
    //
     ...
     *
     */
    public XMacdIndicator() {
        super(IndicatorType.MACD, 100);
    ...
    protected void doSetup(final XChartContext context, Canvas canvas) {
        ...
        final XMacdConfig c = model.getConfig().getIndicator(type);
    ...
    protected void doEval(final XChartContext context, Canvas canvas) {
        ...
        //
        final int p2 = c.getPeriod(2);
        final int size = window.size();
        final XMacdIndex i = new XMacdIndex(model);
        final int max = max(c.getPeriod(0), c.getPeriod(1));
        final float p[] = model.getClosePrices(size + (max - 1) + (p2 - 1), 0);
        i.setLines(macd(p, 0, p.length - 1, c.getPeriod(0), c.getPeriod(1), p2, size));
        window.addIndex(i);
    }
    ...
    protected void doDraw(final XChartContext context, Canvas canvas) {
        ...
        final XMacdConfig c = model.getConfig().getIndicator(type);
        final XMacdIndex i = window.getIndex(type); if(i == null) return;
        ...
        final double unitB = b.getNetHeight() / XChartUtils.subtract(maxB, minB);
        for(int j = 0, size = window.size(); j < size; j++) {
            final int k = size - j - 1;
            final float x1 = wbr - j * sw + sc;
            float x2 = x1 - sw; if (x2 < bl) break;
            if(c.isVisible(1) && !isNaN(curr1 = i.getLine1(k))) {
                final double y0 = bb - (0f - minB) * unitB;
                final double y1 = bb - (curr1 - minB) * unitB;
                if(curr1 < 0d) line10.adds(x2, y0, x2, (float)y1);
                if(curr1 > 0d) line11.adds(x2, y0, x2, (float)y1);
            }
            if(c.isVisible(0)) {
        ...
        this.paint1.setColor(theme.getProperty(CHART_CANDLE_COLOR_DESC, Color.RED));
        if(c.isVisible(1)) XChartUtils.drawLines(line10, canvas, this.paint1, true);
        this.paint1.setColor(theme.getProperty(CHART_CANDLE_COLOR_ASC, Color.GREEN));
        if(c.isVisible(1)) XChartUtils.drawLines(line11, canvas, this.paint1, true);
        ...
        if(theme.getProperty(INDICATOR_CURSOR, true) && cursor > 0) {
            ...
            {
                String p1 = isNaN(curr0 = i.getLine0(k)) ? "-" : format1(curr0, i.getDigits());
                String n1 = c.getName() + " (" + c.getPeriod(0) + "," + c.getPeriod(1) + ")";
                cursors.add(new Cursor(n1, p1, this.paint0));
            }
            if(c.isVisible(2)) {
                final String p2 = isNaN(curr2 = i.getLine2(k)) ? "-" : format1(curr2, i.getDigits());
