...
import cn.nextop.erebor.mid.app.mvc.support.widget.xchart.render.impl.painter.indicator.config.XEmaConfig;
import cn.nextop.erebor.mid.app.mvc.support.widget.xchart.render.impl.painter.indicator.index.XEmaIndex;
...
import static cn.nextop.erebor.mid.app.mvc.support.widget.xchart.formula.XChartFormula.ema;
...
public class XEmaIndicator extends AbstractXIndicator {
    //
     ...
     *
     */
    public XEmaIndicator() {
        super(IndicatorType.EMA, 100);
    ...
    protected void doSetup(final XChartContext context, Canvas canvas) {
        ...
        final XEmaConfig c = model.getConfig().getIndicator(type);
    ...
    protected void doEval(final XChartContext context, Canvas canvas) {
        ...
        final XEmaIndex i = new XEmaIndex(model);
        ...
        if(c.isVisible(0)) i.setLine0(ema(p, from0, p.length - 1, c.getPeriod(0), size));
        if(c.isVisible(1)) i.setLine1(ema(p, from1, p.length - 1, c.getPeriod(1), size));
        if(c.isVisible(2)) i.setLine2(ema(p, from2, p.length - 1, c.getPeriod(2), size));
        window.addIndex(i);
    }
    ...
    protected void doDraw(final XChartContext context, Canvas canvas) {
        ...
        final XEmaConfig c = model.getConfig().getIndicator(type);
        final XEmaIndex i = window.getIndex(type); if(i == null) return;
